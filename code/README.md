...
- **首次运行即自动生成完整演示数据**（克隆后无需任何手动配置，开箱即用）：
  - 默认管理员 `admin / 123456`
  - 3 个充电站 + 18 个电桩（含少量"在用/故障"样例）
  - **近 30 天固定演示订单**（约 160 笔，确定性生成：金额/电量分布固定；以运行日为基准往前 30 天滚动，保证今日/本月/总营收和 7/30 日趋势图始终有数据）
...
- **销售业绩 / 大屏没数据**：确认服务端启动日志出现"已生成近30天固定演示订单数据"；若订单表已有历史数据不会重复生成，可进管理端**销售业绩页点"生成演示数据"按钮**强制重建，或删掉 `test.db` 让服务端重建。
