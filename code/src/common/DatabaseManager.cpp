...
void DatabaseManager::seedDefaultData()
{
    ...
    for (const StationSeed &s : seeds) {
        ...
        for (int i = 0; i < s.pileCount; ++i) {
            const int type = (i % 2 == 0) ? 0 : 1;          // 0-快充 1-慢充
            const double power = (type == 0) ? 60.0 : 7.0;  // kW
            int status = 0;                                 // 默认闲置
            if (codeSeq % 5 == 0) status = 1;               // 示例: 部分在用
            if (codeSeq % 7 == 0) status = 2;               // 示例: 少量故障
